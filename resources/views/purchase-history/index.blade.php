    ...
    <div class="p-8">
        ...
        <div class="flex items-start justify-between mb-6">
            <div>
                ...
                @if ($customer)
                    <p class="text-sm text-gray-500 mt-2">
                        ...
                        &middot;
                        <a href="{{ route('purchase-history.index') }}" class="text-navy underline hover:no-underline">View all
                            customers' orders</a>
                    </p>
                @endif
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('customers.index') }}"
                    class="w-9 h-9 rounded-full bg-black text-white flex items-center justify-center hover:opacity-80 transition">
        ...
        @forelse ($orders as $order)
            <div class="border border-gray-200 rounded-xl p-5 mb-4">
                <form method="GET" action="{{ route('purchase-history.index') }}"
                    class="flex items-start justify-between mb-4 flex-wrap gap-3">
                    <div>
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-700 mt-1">
                                        ...
                                        &nbsp;&nbsp;
                                        <span class="text-blue-600 font-medium">Price</span> : <span class="font-semibold">Php
                                            {{ number_format($item->price, 2) }}</span>
                                    </p>
                                </div>
                            </div>
                            <p
                                class="text-center text-sm font-medium {{ $order->status === 'delivered' ? 'text-green-600' : 'text-amber-500' }}">
                ...
                @if ($order->status === 'delivered')
                    <div class="flex items-center justify-between pt-4 mt-2 border-t border-gray-100">
                        <p class="text-sm text-gray-700">Payment is Successful!</p>
                ...
                @elseif ($order->status === 'cancelled')
                    <div class="pt-4 mt-2 border-t border-gray-100 text-center">
                        <p class="text-sm text-gray-700">Order Cancelled</p>
                    </div>
                @else
                    <div class="flex items-center justify-between pt-4 mt-2 border-t border-gray-100">
                        <p class="text-sm text-gray-700">Payment Pending / In Progress</p>
                        <p class="bg-gray-100 rounded-full px-5 py-2 text-sm font-semibold text-gray-800">
                            Total Price: Php {{ number_format($order->amount, 2) }}
                        </p>
                    </div>
