...
@section('content')
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div class="lg:col-span-2 bg-white rounded-xl p-5 shadow-sm card-hover"
                x-data="{ open: false, range: 'This Week' }">
                <div class="flex items-center justify-between mb-2">
                    <h2 class="font-semibold text-slate-800">Sales Overview</h2>
                    <div class="relative">
                        <button @click="open = !open"
                            class="text-sm px-3 py-1.5 rounded-lg border border-slate-200 flex items-center gap-1 hover:bg-slate-50 transition-colors">
                        ...
                        <div x-show="open" @click.outside="open = false" x-transition x-cloak
                            ...
                            <button @click="range = 'This Week'; open = false; updateSalesChart('This Week')"
                                class="w-full text-left px-3 py-2 text-sm hover:bg-slate-50">This Week</button>
                            <button @click="range = 'This Month'; open = false; updateSalesChart('This Month')"
                                class="w-full text-left px-3 py-2 text-sm hover:bg-slate-50">This Month</button>
                            <button @click="range = 'This Year'; open = false; updateSalesChart('This Year')"
                                class="w-full text-left px-3 py-2 text-sm hover:bg-slate-50">This Year</button>
                        </div>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach ($latestTickets as $ticket)
                                <tr class="border-b border-slate-50 hover:bg-slate-50">
                                    ...
                                    <td class="py-2.5">
                                        <span
                                            class="badge-in text-xs px-2.5 py-1 rounded-full font-medium {{ $ticket->statusBadgeClasses() }}">{{ $ticket->statusLabel() }}</span>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
