...
@section('content')
    ...
    <div class="kpi-row">
        <div class="card kpi-clickable accent-total" onclick="openKpiModal('total')">
            <div class="card-icon icon-total">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                    ...
                    <path
                        d="M12 7v10M9 9.5c0-1.1 1.34-2 3-2s3 .9 3 2c0 1.5-1.5 1.8-3 2.2-1.5.4-3 .7-3 2.3 0 1.1 1.34 2 3 2s3-.9 3-2"
            ...
            <div class="progress-caption">{{ $totalSales['percent'] }}% of ₱{{ number_format($totalSales['target'], 2) }}
                monthly target reached</div>
            ...
            <div class="breakdown-legend">
                @foreach ($totalSales['breakdown'] as $b)
                    <span><span class="dot" style="background: {{ $b['color'] }}"></span>{{ $b['category'] }} ·
                        {{ $b['percent'] }}%</span>
                @endforeach
            </div>
        </div>
        ...
        <div class="card gauge-card kpi-clickable accent-target" onclick="openKpiModal('target')">
            ...
            <div class="gauge-wrap">
                <svg width="110" height="110" viewBox="0 0 110 110">
                    ...
                    <circle cx="55" cy="55" r="46" fill="none" stroke="#1FAE6C" stroke-width="12" stroke-dasharray="289"
                        stroke-dashoffset="{{ 289 - (289 * ($totalSales['percent'] ?? 0) / 100) }}" stroke-linecap="round"
        ...
        <div class="card kpi-clickable accent-forecast" onclick="openKpiModal('forecast')">
            <div class="card-icon icon-forecast">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                    <path d="M4 16l5-6 4 3 7-9" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path d="M15 4h5v5" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        stroke-linejoin="round" />
                </svg>
            </div>
            ...
            <svg class="sparkline" viewBox="0 0 260 46" preserveAspectRatio="none">
                <polyline points="0,36 40,28 80,32 120,14 160,18" fill="none" stroke="#1E2A6E" stroke-width="3"
                    stroke-linecap="round" />
                <polyline points="160,18 200,10 260,4" fill="none" stroke="#1FAE6C" stroke-width="3" stroke-dasharray="5,5"
                    stroke-linecap="round" />
            </svg>
            <div class="forecast-sub">Best case ₱{{ number_format($forecast['best']) }}K · Worst case
                ₱{{ number_format($forecast['worst']) }}K</div>
        </div>
    </div>
    ...
    <div class="tab-panel active" id="panel-product">
        <div class="report-card">
            ...
            <div class="legend" style="margin-bottom:10px;">
                @foreach ($products->pluck('category')->unique() as $cat)
                    <span><span class="dot"
                            style="background: {{ $categoryColors[$cat] ?? '#94A3B8' }}"></span>{{ $cat }}</span>
                @endforeach
            </div>
        ...
        <div class="report-card">
            <h3>Product Performance <span style="font-weight:500; color:var(--sub); font-size:12.5px;">— showing top
                    {{ $products->take(3)->count() }} of {{ $products->count() }}</span></h3>
            <table>
                <tr>
                    <th>Product</th>
                    <th>Qty Sold</th>
                ...
                @foreach ($products->take(3) as $i => $product)
                    <tr>
                        <td>
                            <div class="name-cell"><span class="rank-badge">{{ $i + 1 }}</span>{{ $product['name'] }}</div>
                        </td>
                        <td>{{ $product['qty'] }}</td>
        ...
        <div class="report-card">
            <div class="insights-head">
                <h3>💡 Actionable Insights — Product</h3>
            </div>
            <div class="insight-grid">
                @foreach ($insights['product'] as $item)
                    <div class="insight-card {{ $item['type'] }}">
                        <span class="insight-tag">{{ $item['tag'] }}</span>
                        <p class="insight-text">{{ $item['text'] }}</p>
                        <a href="{{ $item['link'] }}" class="cta-btn"
                            style="text-decoration:none; text-align:center;">{{ $item['btn'] }}</a>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <div class="tab-panel" id="panel-regional">
            ...
            <div class="region-stats-row">
                @foreach ($regions as $region)
                    <div class="region-stat-box" id="region-card-{{ $region['name'] }}"
                        onmouseenter="highlightRegion('{{ $region['name'] }}')"
                        onmouseleave="unhighlightRegion('{{ $region['name'] }}')">
                        <div class="region-stat-top">
                            <span class="dot" style="background:{{ $region['color'] }}"></span>
                            <span class="region-card-name">{{ $region['name'] }}</span>
                        </div>
                        <div class="region-stat-line"><span class="stat-label">Best Day</span><span class="stat-value"
                                id="stat-best-{{ $region['name'] }}">—</span></div>
                        <div class="region-stat-line"><span class="stat-label">Average</span><span class="stat-value"
                                id="stat-avg-{{ $region['name'] }}">—</span></div>
                        <div class="region-stat-line"><span class="stat-label">Period Total</span><span class="stat-value"
                                id="stat-total-{{ $region['name'] }}">—</span></div>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="report-card">
            ...
            <table>
                ...
                @foreach ($regions as $region)
                    ...
                    <tr>
                        <td>
                            <div class="name-cell"><span class="rank-badge"
                                    style="background:{{ $region['color'] }}22; color:{{ $region['color'] }};">{{ substr($region['name'], 0, 1) }}</span>{{ $region['name'] }}
                            </div>
                        </td>
        ...
        <div class="report-card">
            <div class="insights-head">
                <h3>💡 Actionable Insights — Regional</h3>
            </div>
            <div class="insight-grid">
                @foreach ($insights['regional'] as $item)
                    <div class="insight-card {{ $item['type'] }}">
                        <span class="insight-tag">{{ $item['tag'] }}</span>
                        <p class="insight-text">{{ $item['text'] }}</p>
                        <a href="{{ $item['link'] }}" class="cta-btn"
    ...
    <div class="tab-panel" id="panel-rep">
        ...
        <div class="report-card spotlight-card">
            ...
            <div class="spotlight-info">
                ...
                <div class="legend" style="margin-bottom:0;">
                    <span><span class="dot"
                            style="background:{{ $regions->keyBy('name')[$topRep['region']]['color'] }}"></span>{{ $topRep['region'] }}</span>
                    <span>{{ $topRep['deals'] }} deals closed</span>
                </div>
            </div>
            <div class="spotlight-stats">
                <div class="spotlight-revenue">₱{{ number_format($topRep['revenue']) }}</div>
                <div class="progress-caption">{{ $topRep['quotaPercent'] }}% of ₱{{ number_format($topRep['quota']) }} quota
                </div>
            </div>
        </div>
        ...
        <div class="report-card">
            ...
            <table>
                ...
                @foreach ($reps->sortByDesc('revenue')->take(3) as $i => $rep)
                    ...
                    <tr>
                        <td>
                            <div class="name-cell"><span class="rank-badge">{{ $medals[$i] ?? $i + 1 }}</span></div>
                        </td>
                        <td>
                            <div class="name-cell">{{ $rep['name'] }}</div>
                        </td>
                        <td><span class="region-tag"><span class="dot"
                                    style="background:{{ $regionColor }}"></span>{{ $rep['region'] }}</span></td>
                        <td>₱{{ number_format($rep['revenue']) }}</td>
                        <td>
                            <div class="quota-bar-track">
                                <div class="quota-bar-fill"
                                    style="width:{{ min($rep['quotaPercent'], 100) }}%; background:{{ $rep['quotaPercent'] >= 100 ? '#1FAE6C' : '#3B82F6' }};">
                                </div>
                            </div>
        ...
        <div class="report-card">
            <div class="insights-head">
                <h3>💡 Actionable Insights — Representative</h3>
            </div>
            <div class="insight-grid">
                @foreach ($insights['rep'] as $item)
                    <div class="insight-card {{ $item['type'] }}">
                        <span class="insight-tag">{{ $item['tag'] }}</span>
                        <p class="insight-text">{{ $item['text'] }}</p>
                        <a href="{{ $item['link'] }}" class="cta-btn"
    ...
    <div class="modal-overlay" id="kpi-modal">
        <div class="modal-box modal-box-wide">
            ...
            <div class="kpi-modal-panel" id="kpi-panel-total">
                ...
                <p class="modal-sub">₱{{ number_format($totalSales['value'], 2) }} this month, +{{ $totalSales['change'] }}%
                    vs last month</p>
                ...
                <p class="modal-note"><a href="{{ route('reports.sales.products') }}"
                        style="color:var(--green); font-weight:700;">View full per-product breakdown →</a></p>
            </div>

            {{-- TARGET DETAIL --}}
            <div class="kpi-modal-panel" id="kpi-panel-target" style="display:none;">
                <h3>Target Achievement</h3>
                <p class="modal-sub">You're at {{ $totalSales['percent'] }}% of this month's
                    ₱{{ number_format($totalSales['target'], 2) }} target.</p>
            ...
            <div class="kpi-modal-panel" id="kpi-panel-forecast" style="display:none;">
                ...
                <p class="modal-sub">Projected at ₱{{ number_format($forecast['value'], 2) }}, +{{ $forecast['change'] }}%
                    vs this month.</p>
                <div class="modal-forecast-range">
                    <div class="modal-stat-box">
                        <span class="modal-stat-label">Worst Case</span>
                        <span class="modal-stat-value"
                            style="color:var(--red)">₱{{ number_format($forecast['worst']) }}K</span>
                    </div>
                    <div class="modal-stat-box">
                        <span class="modal-stat-label">Best Case</span>
                        <span class="modal-stat-value"
                            style="color:var(--green)">₱{{ number_format($forecast['best']) }}K</span>
                    </div>
                </div>
                <p class="modal-note">Forecast is projected from recent monthly growth trend. This becomes more accurate
                    once real historical sales data is connected.</p>
            </div>
        </div>
    </div>
        ...
        <div class="modal-box" style="text-align:left;">
            ...
            <form action="{{ route('reports.sales.export') }}" method="GET">
                <div style="margin-bottom:16px;">
                    <label
                        style="display:block; font-size:12.5px; font-weight:700; color:var(--sub); margin-bottom:6px;">Report</label>
                ...
                <div style="margin-bottom:22px;">
                    <label
                        style="display:block; font-size:12.5px; font-weight:700; color:var(--sub); margin-bottom:6px;">File
                        Format</label>
                ...
                <div style="display:flex; gap:10px; justify-content:center;">
                    <button type="button" class="modal-close"
                        style="background:#fff; color:var(--navy); border:1px solid var(--line);"
