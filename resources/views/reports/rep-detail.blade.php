...
@section('detail-content')
    ...
    <div class="report-card">
        <div class="filter-row">
            <input type="text" id="rep-search" class="search-input" placeholder="Search representatives..."
                onkeyup="filterReps()">
            <div class="chip-row" id="region-chips">
                ...
                @foreach ($regions as $region)
                    <button class="chip" data-region="{{ $region['name'] }}"
                        onclick="setRegionFilter('{{ $region['name'] }}', this)">{{ $region['name'] }}</button>
                @endforeach
            </div>
        </div>
        ...
        <table>
            ...
            <tbody id="rep-table-body">
                @foreach ($reps->sortByDesc('revenue')->values() as $i => $rep)
                    @php $regionColor = $rep['regionColor'] ?? '#94A3B8'; @endphp
                    <tr class="rep-row" data-region="{{ $rep['region'] }}" data-name="{{ strtolower($rep['name']) }}"
                        onclick="toggleExpand('rep-{{ $i }}')">
                            ...
                            <div class="expand-panel">
                                <div class="expand-col">
                                    ...
                                    <svg viewBox="0 0 200 50" class="mini-sparkline" preserveAspectRatio="none">
                                        <polyline points="{{ $pts }}" fill="none" stroke="{{ $regionColor }}" stroke-width="3"
                                        ...
                                        <div class="mini-bar-row">
                                            ...
                                            <div class="mini-bar-track">
                                                <div class="mini-bar-fill"
                                                    style="width:{{ $p['percent'] }}%; background:{{ $regionColor }};"></div>
                                            </div>
                                ...
                                <div class="expand-col">
                                    ...
                                    @foreach ($rep['coverage'] as $regionName => $pct)
                                        ...
                                        <div class="mini-bar-row">
                                            ...
                                            <div class="mini-bar-track">
                                                <div class="mini-bar-fill" style="width:{{ $pct }}%; background:{{ $rc }};"></div>
                                            </div>
